...
public class Cam extends Component {
  ...
  public BufferedImage render() {
    ...
    for (SpriteRenderer spriteRenderer : spriteRenderers) {
      for (int w = 0; w < spriteRenderer.getSprite().getWidth(); w++) {
        for (int h = 0; h < spriteRenderer.getSprite().getHeight(); h++)
          if (w < resolution.x && h < resolution.y) {
            try {
              Vector2 screenCenterPosition = new Vector2(resolution.x / 2,resolution.y / 2);
              float pixelsPerUnit = (size.x * size.y) / (resolution.x * resolution.y);
              ...
              frame.setRGB(w + (int)pixelPosition.x - (int)spriteRenderer.getDefaultSize().x, h +(int)pixelPosition.y - (int)spriteRenderer.getDefaultSize().y, spriteRenderer.getSprite().getRGB(w, h));
